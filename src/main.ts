...
import { AuthMiddleware, errorLogger, handleError, handleSuccess, schemaParser } from "./utils.ts";
import { __DEV__, API_PORT, BASE_URL } from "./config.ts";
import { CreatedUrl, createUrl, getUrl } from "./actions.ts";
...
ApiRouter.post<string>(`/new`, async (ctx: RouterContext<string>) => {
  try {
    const { body: payload } = await schemaParser(
      CreateSchema,
      ctx,
      )
